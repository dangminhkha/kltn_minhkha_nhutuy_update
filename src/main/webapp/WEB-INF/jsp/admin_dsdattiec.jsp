...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
     ...
     <!-- Site Metas -->
    <title>Danh sách đặt tiệc</title>  
...
<li class="nav-item dropdown">
	...
	<div class="all-page-title page-breadcrumb">
		<div class="container text-center">
			<div class="row">
				<div class="col-lg-12">
					<h1>QUẢN LÝ DANH SÁCH ĐẶT TIỆC</h1>
				</div>
			</div>
		</div>
	</div>
			...
			<div class="row inner-menu-box">
				<div class="col-3">
					<div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
						<br><br><br>
						<a class="nav-link" href="/dsdattiecadmin">Đang chờ duyệt</a>
						<a class="nav-link" href="/dsdattiecadmin?trangthai=DADUYET">Đã duyệt</a>
						<a class="nav-link" href="/dsdattiecadmin?trangthai=DATHANHTOAN">Đã thanh toán</a>						
						<a class="nav-link" href="/dsdattiecadmin?trangthai=HUY">Đã hủy</a>
					...
					<div class="row">
						...
						<div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
							...
							<table class="table">
								...
								<c:forEach var="ttdt" items="${dsTTDatTiec }">
							<tr>
								<td>${ttdt.nguoidung.ten }</td>
								<td>${ttdt.catochuc } <br> ${ttdt.ngaytochuc }</td>
								<td>${ttdt.sanh.ten }</td>
								<td>${ttdt.loai }</td>
								<td><a class="btn btn-success" href="/adminxemttdt?id=${ttdt.id }">Xem<a></a></td>
							</tr>
							</c:forEach>
							</table>
							...
							<a class="page-style1" href="dsdattiecadmin?trangthai=${trangthai }&page=${pageTruoc}">Trước </a>
							<c:if test="${page1!=null}">
							<a class="page-style1" href="dsdattiecadmin?trangthai=${trangthai }&page=${page1}">${page1 }</a></c:if>
							<c:if test="${page2!=null}">
							<a class="page-style1" href="dsdattiecadmin?trangthai=${trangthai }&page=${page2}">${page2 }</a></c:if>
							<a class="page-style" href="">${pageHienTai }</a>
							<c:if test="${page4!=null}">
							<a class="page-style1" href="dsdattiecadmin?trangthai=${trangthai }&page=${page4}">${page4 }</a></c:if>
							<c:if test="${page5!=null}">
							<a class="page-style1" href="dsdattiecadmin?trangthai=${trangthai }&page=${page5}">${page5 }</a></c:if>
							<a class="page-style1" href="dsdattiecadmin?trangthai=${trangthai }&page=${pageSau}">Sau </a>
						</div>
						</div>
					</div>
				</div>
