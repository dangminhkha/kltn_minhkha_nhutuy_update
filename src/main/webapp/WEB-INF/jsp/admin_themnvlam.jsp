...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
     ...
     <!-- Site Metas -->
    <title>Chi tiết nhóm thời vụ</title>  
	...
	<div class="contact-box">
		<div class="container">
			...
			<div class="submit-button text-center">	
				<h3>Thêm nhân viên làm ngày ${timelam }</h3> </div>
				<table class="table">
					<tr> 
						<td>Tên</td>
						<td>Số điện thoại</td>
					...
					<c:forEach var="nv" items="${dsNVthoivu }"> 
						<tr> 
						<td>${nv.ten }</td>
						...
						<td>${nv.tienluong }</td>
						<td>
						<c:if test="${nv.dachamcong==true }">
							<a  class="btn btn-warning">Đã Thêm</a> 
						</c:if>
						<c:if test="${nv.dachamcong==false }">
						<div class="col-lg-12">
						<form action="themnvlam" method="get">
								<div class="row no-gutters">
								<input type="hidden" name="id" id="id" value="${nv.id }" >
								<input type="hidden" name="ngaylam" id="ngaylam" value="${ngaylam }" >
								
									
									  <div class="col-lg-8">
									  <select class="form-control" name="calam" id="calam">
				...
				<div class="submit-button text-center">
				<a href="themnvthoivu" class="btn btn-common">Thêm Nhân Viên</a>
				</div>
				</div>
			</div>
		</div>
