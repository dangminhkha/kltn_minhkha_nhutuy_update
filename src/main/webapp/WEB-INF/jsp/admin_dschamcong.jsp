...
<li class="nav-item dropdown">
	...
	<div class="contact-box">
		<div class="container">
			...
			<div class="col-lg-6">
						...
						<div class="col-lg-6">
							<div class="heading-title text-center">
								<form action="chamcong" method="get">
								<div class="row no-gutters">
								<div class="col-lg-5"><h3>Chọn ngày</h3> </div>
								<div class="col-lg-7">
									<input class="form-control" id="ngaycong" name="ngaycong"  onchange="this.form.submit()" type="date">
									</div>
			...
			<h3>Bảng chấm công ngày ${ngaychamcong }</h3> </div>
			<table class="table">
				<tr> 
					<td>Tên nhân viên</td>
					<td>Số điện thoại</td>
					<td>Email</td>
					<td>Chấm công cho hôm nay</td>
				</tr>
				<c:forEach var="ds" items="${dsnvchamcong }"> 
				<tr> 
					<td>${ds.ten }</td>
					<td>${ds.sodienthoai }</td>
					<td>${ds.email }</td>
					<td>
						<c:if test="${ds.dachamcong==true }">
							<a  class="btn btn-warning">Đã chấm công</a> 
						</c:if>
						<c:if test="${ds.dachamcong==false }">
						<c:if test="${tongdsluong==0 }"><a href="chamcongnhanvien?id=${ds.id }&ngaycong=${ngaycong }" class="btn btn-primary">Chấm công</a> </c:if> </c:if>
						<a href="xembangcongnv?id=${ds.id }&nam=${namhientai }&thang=${thanghientai }" class="btn btn-success">Xem Bảng Công</a>
					</td>
				</tr>
			...
			<c:if test="${tongdsluong==0 }"> 
			
				<div class="submit-button text-center">
				<a href="formtinhluong?nam=${namhientai }&thang=${thanghientai }" class="btn btn-common">Tính Lương</a>
				</div>
				</c:if>
				...
				<div class="submit-button text-center">
				<a href="bangluong?nam=${namhientai }&thang=${thanghientai }" class="btn btn-common">Xem Bảng Lương</a>
				</div>
				</c:if>
			</div> </div>
		</div>
