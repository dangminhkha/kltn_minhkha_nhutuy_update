<%@page import="com.example.demo.dtos.CTHoaDonDTO"%>
...
<li class="nav-item dropdown">
	...
	<div class="menu-box">
		<div class="container">
			<div id="GFG" > 
			<div align="center" class="row inner-menu-box">
				<div class="col-12">
				<div class="row">
						<div class="col-lg-12">
				...
				<table class="table">
								<tr> 
									<td>Khách Hàng </td>
									<td>${hoadonthanhtoan.ttDatTiec.nguoidung.ten } </td>
								</tr>
								<tr> 
									<td>Số điện thoại </td>
									<td>${hoadonthanhtoan.ttDatTiec.nguoidung.sodienthoai } </td>
								</tr>
								<tr> 
									<td>Email </td>
									<td>${hoadonthanhtoan.ttDatTiec.nguoidung.email } </td>
								</tr>
								<tr> 
									<td>Ngày tổ chức </td>
									<td>${hoadonthanhtoan.ttDatTiec.ngaytochuc } </td>
								</tr>
								<tr> 
									<td>Ca tổ chức </td>
									<td>${hoadonthanhtoan.ttDatTiec.catochuc } </td>
								</tr>
								
								<tr> 
									<td>Nhân viên thanh toán </td>
									<td>${userDangNhap.ten } </td>
								</tr>
								
								<tr> 
									<td>Thời gian thanh toán </td>
									<td>${hoadonthanhtoan.ngaythanhtoanFM } </td>
								</tr>
							</table>
							
				
							<form action="thanhtoanhoadon" method="get">
							<table  style="width: 100%" border="1"  >
								...
								<%List<CTHoaDonDTO> dsCtHd =(List<CTHoaDonDTO>) session.getAttribute("cthoadonthanhtoan"); 
								for(int i = 0;i<dsCtHd.size();i++){
									if(dsCtHd.get(i).getCombo()!=null){
								%>
								<tr>
									<td><%=dsCtHd.get(i).getCombo().getTen() %></td>
									<td><%=dsCtHd.get(i).getCombo().getGiaFM() %></td>
									<td align="center"><%=dsCtHd.get(i).getSoluong() %></td>
									<td><%=dsCtHd.get(i).getGiaFM() %></td>
								</tr>
								<%}else if(dsCtHd.get(i).getMon()!=null){%>
								<tr>
									<td><%=dsCtHd.get(i).getMon().getTen() %></td>
									<td><%=dsCtHd.get(i).getMon().getGiaFM() %></td>
									<td align="center"><%=dsCtHd.get(i).getSoluong() %></td>
									<td><%=dsCtHd.get(i).getGiaFM() %></td>
								</tr>
								<%}else{ %>
								<tr>
									<td><%=dsCtHd.get(i).getTen() %></td>
									<td><%=dsCtHd.get(i).getGiaDon()%></td>
									<td align="center" ><%=dsCtHd.get(i).getSoluong() %></td>
									<td><%=dsCtHd.get(i).getGiaFM() %></td>
								</tr>
								...
								<%}} %>
								<tr>
									<td >Tổng giá hóa đơn</td>
									<td> </td>
									<td> </td>
						
									<td>${hoadonthanhtoan.giaFM }</td>
								</tr>
								<tr>
									<td colspan="3">Tiền cọc</td>
									<td>${hoadonthanhtoan.ttDatTiec.tiencocFM }</td>
								</tr>
								<tr>
									<td colspan="3">Tổng trả</td>
									<td>${tientra }</td>
								</tr>
							</table>
		...
		<div class="col-lg-12"> 
			<div class="submit-button text-center">
				<a href="thanhtoanhoadon" class="btn btn-common"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Thanh Toán</a>
				<a href="" class="btn btn-success" onclick="printDiv()"><i class="fa fa-print" aria-hidden="true"></i> In Hóa Đơn </a>  
			</div>
		</div>
	</div>
	</div>
