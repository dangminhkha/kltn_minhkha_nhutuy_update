			...
			<div class="row inner-menu-box">
				...
				<div class="row">
					...
					<div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
						<br><a class="nav-link" href="/tiecdadat">Danh sách chờ duyệt</a>
						<a class="nav-link" href="/dsdaduyet">Danh sách đã duyệt</a>
						<a class="nav-link" href="/dsdatochuc">Danh sách đã tổ chức</a>
						<a class="nav-link" href="/dsdahuy">Danh sách đã hủy</a>
					</div>
				</div>
				<div class="col-9">
				<div class="row">
					...
					<div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
							...
							<tr>
								...
								<td>${ttdt.soban }</td>
								<td>${ttdt.loai }</td>
								<td><a class="btn btn-success" href="/xemttdt?id=${ttdt.id }">Xem<a></a></td>
							</tr>
							</c:forEach>
							</table>
							...
							<div class="submit-button text-center">						
								<a class="page-style1" href="${urldattiec }?page=${pageTruoc}">Trước </a>
								<c:if test="${page1!=null}">
								<a class="page-style1" href="${urldattiec }?page=${page1}">${page1 }</a></c:if>
								<c:if test="${page2!=null}">
								<a class="page-style1" href="${urldattiec }?page=${page2}">${page2 }</a></c:if>
								<a class="page-style" href="">${pageHienTai }</a>
								<c:if test="${page4!=null}">
								<a class="page-style1" href="${urldattiec }?page=${page4}">${page4 }</a></c:if>
								<c:if test="${page5!=null}">
								<a class="page-style1" href="${urldattiec }?page=${page5}">${page5 }</a></c:if>
								<a class="page-style1" href="${urldattiec }?page=${pageSau}">Sau </a>
							</div>
						</div>
				</div>
			</div>
