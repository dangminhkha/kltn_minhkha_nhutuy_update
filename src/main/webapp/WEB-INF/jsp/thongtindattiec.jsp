...
<!-- Site Metas -->
<title>Thông tin đặt tiệc</title>
			...
			<div class="row inner-menu-box">
				...
				<div class="col-12">
					<div class="tab-content" id="v-pills-tabContent">
							...
							<div class="tab-pane fade show active" id="v-pills-home"
								...
								<div class="row">
									...
									<table class="table">
										<tr>
											<td>Ngày tổ chức</td>
											<td>${ttdattiec.ngaytochuc }</td>
										</tr>
										<tr>
											<td>Ca</td>
											<td>${ttdattiec.catochuc }</td>
										</tr>
										<tr>
											<td>Số khách</td>
											<td>${ttdattiec.sokhach }</td>
										</tr>
										<tr>
											<td>Số bàn</td>
											<td>${ttdattiec.soban }</td>
										</tr>
										<tr>
											<td>Loại</td>
											<td>${ttdattiec.loai }</td>
										</tr>
										<tr>
											<td>Tên Sảnh</td>
											<td>${ttdattiec.sanh.ten }</td>
										</tr>
										<tr>
											<td>Combo</td>

											<td>Tên: ${ttdattiec.combo.ten }<br>
											Giá: ${ttdattiec.combo.giaFM } cho 1 bàn
												<p>Danh sách món:</p> <c:forEach var="ct"
													items="${ctcombodat}">
													<p>${ct.stt } - ${ct.mon.ten }</p>
												</c:forEach>
											</td>
										</tr>
										<tr>
											...
											<c:forEach var="nuoc" items="${dsnuocuong }">
												<tr>
												<td>
												${nuoc.mon.ten }</td>
												<td>${nuoc.mon.giaFM }/1 </td>
												<td> <a href="xoanuocdattiec?id=${nuoc.mon.id }">Xóa</a> </td>
												</tr>
											</c:forEach>
											</table>
											</td>
						...
						<p class="color-red">${thongbao } </p>
							<a href="/quaylaichonmon" class="btn btn-info">Quay Lại </a> 
							<%TTDatTiecDTO ttdt =(TTDatTiecDTO) session.getAttribute("ttdattiec"); %>
							<%if(ttdt.getCtNuocDatTiecs().size()==0){ %>
							<br><p class="color-red">Bạn chưa chọn nước vui lòng quay lại để chọn nước</p>
							<%}else{%>
							<a href="/luudattiec" class="btn btn-common">Đặt Tiệc </a>
							<%} %>
						</div>
					</div>
				</div>
