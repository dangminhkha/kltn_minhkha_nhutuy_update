					...
					<div class="row">
						...
						<div class="col-lg-6">
							<div class="heading-title text-center">
								<form action="admintimsanh" method="get">
						...
						<div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
							<div class="row">
								<p>${dsrong }</p>
							<c:forEach var="sanh" items="${sanh }">
								<div class="col-lg-4 col-md-6 special-grid drinks">
									<div class="gallery-single fix">
										<img src="${sanh.hinh }" class="img-fluid khuongdssanh" alt="Image">
									</div>
									<h1 class="color-Maroon">${sanh.ten }</h1>
									<p>Sức chứa từ ${sanh.sokhachtoithieu } đến ${sanh.sokhachtoida }</p>
									<div class="submit-button text-center">
									<p class="color-red">${thongbaoxoasanh }</p>
										<a href="/xoasanh?id=${sanh.id }" class="btn btn-success"><i class="fa fa-trash"></i> Xóa</a>
							...
							<div class="submit-button text-center">						
								<a class="page-style1" href="qlsanh?page=${pageTruoc}">Trước </a>
								<c:if test="${page1!=null}">
								<a class="page-style1" href="qlsanh?page=${page1}">${page1 }</a></c:if>
								<c:if test="${page2!=null}">
								<a class="page-style1" href="qlsanh?page=${page2}">${page2 }</a></c:if>
								<a class="page-style" href="">${pageHienTai }</a>
								<c:if test="${page4!=null}">
								<a class="page-style1" href="qlsanh?page=${page4}">${page4 }</a></c:if>
								<c:if test="${page5!=null}">
								<a class="page-style1" href="qlsanh?page=${page5}">${page5 }</a></c:if>
								<a class="page-style1" href="qlsanh?page=${pageSau}">Sau </a>
							</div></c:if>
						</div>
					</div>
					<div class="submit-button text-center">
								<a class="btn btn-common" href="/themsanh">Thêm Sảnh</a>
							</div>
				</div>
			</div>
		</div>
